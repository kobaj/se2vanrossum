...
;
;;match-down-to-up
;(check-expect (match-down-to-up 1 2 (transpose
;                                            (nthrdc 
;                                              (- (len (nthcdr (- (+ 1 1) 
;                                                       (cadr (list "gc" 2))) 
...
(check-expect (hill-climbing-solver(list(list "r" "i" "g" "h" "t" "f" "e" "l" "l") ;example game board
                    ...
                    (list (list "l" "e" "f" "t") ; the problem is if there are more than one sol per row, then it only finds the first one and moves on. 
                          (list "r" "i" "g" "h" "t")
                          (list "u" "p")
                          (list "d" "o" "w" "n")
                          (list "t" "e" "s" "t")
